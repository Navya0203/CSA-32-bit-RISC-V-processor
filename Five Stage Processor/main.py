...
#to calculate the twos complement value
...
class register_Pipeline(object):
    def __init__(self):
...
class Core(object):
    def __init__(self, iDir, oDir, imem, dmem):
        ...
        self.regPipeline = register_Pipeline()
        self.Hazards_Check = Hazards_Check()
...
class FiveStageCore(Core):
    ...
    def instructionDecode(self):
        ...
        if not self.state.ID["nop"]:
            ...
            if self.Hazards_Check.hazardRegWrite(
            ...
            if self.Hazards_Check.hazardEX(
            ...
            elif self.Hazards_Check.hazardMEM(
                    self.regPipeline, self.regPipeline.ID_EX["Rs1"]
            ):
                self.regPipeline.ID_EX["instruction"]["Read_data1"] = self.regPipeline.MEM_WB["Wrt_data"]

            if self.regPipeline.ID_EX["instruction"]["imm"] == "X":
                if self.Hazards_Check.hazardEX(self.regPipeline, self.regPipeline.ID_EX["Rs2"]):
                    self.regPipeline.ID_EX["instruction"]["Read_data2"] = self.regPipeline.EX_MEM["ALUresult"]

                elif self.Hazards_Check.hazardMEM(self.regPipeline, self.regPipeline.ID_EX["Rs2"]):
                    self.regPipeline.ID_EX["instruction"]["Read_data2"] = self.regPipeline.MEM_WB["Wrt_data"]
    ...
    def instructionFetch(self):
        ...
        if not self.state.IF["nop"]:
            ...
            if opcode == "1111111":
                ...
                if self.Hazards_Check.hazardLoad(self.regPipeline, instructionReverse[15:20][::-1],
...
if __name__ == "__main__":
    ...
    oDir = os.path.join(os.path.abspath(args.iodir), 'output_nk3696')
